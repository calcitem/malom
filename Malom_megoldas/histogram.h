...
public:
    ...
    // file_name is the name of the .hist (data for gnuplot), .plt and .png
    // files that are generated when calling the appropriate methods
    histogram(vector<double> &v, double min, double max, int n,
              string filename);
    ...
    // calls the above two methods, then calls gnuplot, and then calls xdg-open
    // if open is set to true
