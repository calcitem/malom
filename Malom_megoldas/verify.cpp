...
void verify(id main_id)
{
    ...
    for (int h = 0; h < main_hash->hash_count; h++)
        if (main_sec->get_eval_inner(h).cas() != eval_elem_sym2::Sym) {
            // int ma=-inf;
            eval_elem2 ma = eval_elem2 {
                (sec_val)(virt_loss_val - main_sec->sval),
                0}; // a 0 tavolsag itt valoban minimalis, mert nem valt
                    // elojelet a key1, ha valahol virt_loss_val-bol indult,
                    // tehat a key2 nem lehet negativ
            ...
            for (int i = 0; i < num_chd; i++)
                ...
                // elso sora, tovabba doc-ban "Nincs korong f鰊t probl閙醟"
                // szekcio
                if (chd[i].s != nullptr) {
                    ma = max(ma, negate_eval_elem2(
                                     chd[i].s->hash->hash(chd[i].a).second));
                } else {
                    // ez csakis egy egylepeses nyeres lehet
                    //(a graph_func-ban is szerepel a B+BF feltetel, tehat azert
                    //lesz nullptr, mert nem hozunk letre teljesen vesztes
                    //szektort)
                    REL_ASSERT(main_id.B + main_id.BF <= 3 &&
                               can_close_mill(a, main_id.W, main_id.B,
                                              main_id.WF,
                                              main_id.BF)); // ez a win
                                                            // conditionunk
                    // ma=inf-1;
                    assert(virt_win_val);
                    ma = eval_elem2 {(sec_val)(virt_win_val - main_sec->sval),
                                     1};
                }
            ...
            if (shouldbe != ma) {
                LOG("!!! Verification error !!!  Sector: %s, Hash: %d, Board: "
                    "%lld, shouldbe: (%hd, %d), ma: (%hd, %d)\n",
                    main_sec->id.to_string().c_str(), h, a, shouldbe.key1,
                    shouldbe.key2, ma.key1, ma.key2);
