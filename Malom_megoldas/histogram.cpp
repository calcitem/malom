...
void call_gnuplot(string plt_file, string data_file,
                  string png_filename = "tmp.png", bool open = true,
                  bool del_pltfile = true, bool del_datafile = true)
{
...
histogram::histogram(vector<double> &v, double min, double max, int n,
                     string filename)
    : distr(n + 1)
    , w((max - min) / n)
    , min(min)
    , filename(filename)
    , hist_filename(filename + ".hist")
    , plt_filename(filename + ".plt")
...
histogram::histogram(vector<int> &distr, int min, string filename)
    : histogram {vector<long long>(distr.begin(), distr.end()), min, filename}
{ }
