...
{ // a processzhez tartozo bucketek mappajanak neve
...
big_bucket_elem::big_bucket_elem(short_id sid, int hash, int key2)
    : small_bucket_elem {sid, hash}
    , key2 {key2}
{ }
...
void big_bucket_writer::init()
{
    fopen_s(
        &f,
        ("buckets\\" + bucket_prefix() + "\\" + tostring(key1) + ".bigbucket")
            .c_str(),
        "wb");
...
big_bucket_writer::big_bucket_writer(sec_val key1)
    : key1 {key1}
    , f {0}
{ }
...
small_bucket_writer::small_bucket_writer(int key2)
    : key2 {key2}
{
    errno_t err = fopen_s(&f,
                          ("buckets\\" + bucket_prefix() + "\\small_buckets\\" +
                           tostring(key2) + ".smallbucket")
                              .c_str(),
                          "wb");
    if (err) {
        LOG("!!! error: a %d small_bucket letrehozasa nem sikerult. (%s)", key2,
            ("buckets\\" + bucket_prefix() + "\\small_buckets\\" +
             tostring(key2) + ".smallbucket")
                .c_str());
        perror("Hiba: ");
...
void big_bucket_writer::push(big_bucket_elem e)
{
    ...
    // fwrite(&e, sizeof(e),1,f); //igy a paddeles miatt esetleg tobb byte-ot
    // irna ki
...
{
    bucket_name = "buckets\\" + bucket_prefix() + "\\small_buckets\\" +
                  tostring(key2) + ".smallbucket";
...
small_bucket_elem small_bucket_reader::pop()
{
    ...
    int hash;
    size_t not_eof = _fread_nolock(&hash, sizeof(hash), 1, f);
    assert(not_eof);*/
...
// const queue_elem queue_elem::max = queue_elem{ (char)0, 0, ::val{ -1,
// numeric_limits<int>::max() } }; //(see val::operator<)
const queue_elem queue_elem::max = queue_elem {
    (char)0, 0, ::val {-1, numeric_limits<int>::max()}}; //(see val::operator<)
...
bucket_writer_mgr::bucket_writer_mgr()
{
    ...
    CreateDirectory(
        str2wstr("buckets\\" + bucket_prefix() + "\\small_buckets").c_str(),
        NULL);
    _setmaxstdio(2048);
}
...
big_bucket_reader::big_bucket_reader(big_bucket_writer &bbw)
{
    ...
    string big_bucket_name = "buckets\\" + bucket_prefix() + "\\" +
                             tostring(bbw.key1) + ".bigbucket";
    FILE *f;
    fopen_s(&f, big_bucket_name.c_str(), "rb");
    const int bufsize = 1024 * 1024;
    char *buf = new char[bufsize];
    setvbuf(f, buf, _IOFBF, bufsize);
    pn_vector<small_bucket_writer> sbws(
        [](int i) { return small_bucket_writer(i); });
    while (1) {
        big_bucket_elem e;
    ...
    assert(sbws.size()); // ez azert teljesul mindig, mert ha volt nagy bucket
                         // fajl, akkor lesz kis bucket is
