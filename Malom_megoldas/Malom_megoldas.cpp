...
void progressbar(int a, int b, int sum)
{
    ...
    int ha = int(a * (__int64)(bar_width - 2) / sum),
        hb = int((a + b) * (__int64)(bar_width - 2) / sum), i = 1;
    t[0] = '[';
    while (i <= ha)
        t[i++] = '=';
    while (i <= hb)
...
int main(int argc, char *argv[])
{
    ...
    if (argc < 6) {
        ...
        if (!strcmp(argv[5], "-solve")) {
            mode = solution_mode;
            Log::setup_logfile(run_params.to_string(),
                               "solutionlog"); // ide nem kell a suffix, mert
                                               // azt a Log osztaly intezi
        } else {
            if (!strcmp(argv[5], "-verify")) {
                mode = verification_mode;
                Log::setup_logfile(run_params.to_string(), "verificationlog");
                LOG("Verification mode\n");
            } else if (!strcmp(argv[5], "-analyze")) {
                mode = analyze_mode;
...
void exp3()
{
    ...
    stable_sort(qes.begin(), qes.end(),
                [](queue_elem a, queue_elem b) { return a.val < b.val; });
...
void exp()
{
    ...
    //		tmp+=hash->hash(a); //ez nem a parent-oket hash-eli egyelore, mert
    //ahhoz kene a masik sector hash fuggvenyet is inicializalni (persze ez igy
    //a cache-ek miatt nem igazan jol meri a sebesseget)
    // }
    // cout<<tmp<<endl;
    ...
    //	h0+=10000000; //azert jobb igy tesztelni, mert igy nem fognak egymasra
    //hasonlitani az egymas utani allasok 	h0%=603332730;
    //}
