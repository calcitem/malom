...
bool twin; //(amugy van olyan, hogy nem EKS, de megse twin, pl. standard
           //felrakas paratlan szinten)
...
void collect_sector_family(id u)
{ // berakja a wsectorsba az id-t es a gyerekeit
    wsectors.insert(u);
    auto e = graph_func(u);
...
void phase1()
{
    /*1. minden szektorban végigiterálunk a (nem-sym) állásokon:
            ...
            -queue-ba rakás (ha nem döntetlen (azaz val (foszektorban mindig
       olyan, hogy a key1=win-sval, key2=1)))
    */
    LOG("***phase1***\n");
    long long counter = 0;
    ...
    for (auto it = sector_objs.begin(); it != sector_objs.end(); ++it) {
        ...
        bool alloc = s.wms == -1;
        if (alloc)
            s.allocate(false, parent_ids[s.id]);
...
void phase2()
{
    // 2. ahol a foszektorokban count-0 maradt, átírjuk vesztés valra, és
    // berakjuk a sorba

    LOG("***phase2***\n");
    long long counter = 0;

    if (!(FULL_BOARD_IS_DRAW && main_secs[0]->W == 12 &&
          main_secs[0]->B == 12)) {
        for (auto it = main_secs.begin(); it != main_secs.end(); ++it) {
            Sector &s = **it;
...
void phase3()
{
    // 3. a sor feldolgozása (a sorból kivett állások némelyike
    // gyerekszektorbeli, némelyike foszektorbeli)
    LOG("***phase3***\n");
    {
        long long counter = 0;
        ...
        while (!aq.empty()) {
            ...
            for (int i = 0; i < num_parents; i++) {
                ...
                if (ph.second.cas() == eval_elem2::Count) { // count-ba
                    ...
                    assert(ph.second.value() <= qe.val.undo_negate() ||
                           ph.second.value().key1 == virt_win_val - ps.sval &&
                               qe.val.key1 > 0);
                }
            }
        }
