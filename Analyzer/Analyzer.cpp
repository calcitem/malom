...
pn_vector<long long> akey1_dist; // akey1 distribution  (range will be forced to
                                 // virt_loss_val-virt_win_val)
int read_virt_loss_val = 0, read_virt_win_val = 0;
map<id, int> all_maxvals;

map<id, wdl> read_sm(long long &sym_count, long long &win_count,
                     long long &loss_count, long long &draw_count)
{
    DIR *dir;
    struct dirent *ent;
    if ((dir = opendir(".")) != NULL) {
        ...
        while ((ent = readdir(dir)) != NULL) {
            ...
            if (starts_with(VARIANT_NAME, f_name) &&
                ends_with(string(".analyze") + FNAME_SUFFIX, f_name)) {
                id s;
                sscanf_s(f_name.c_str(), VARIANT_NAME "_%d_%d_%d_%d", &s.W,
                         &s.B, &s.WF, &s.BF);
                ...
                int cur_maxval, cur_sym_count, cur_win_count, cur_draw_count,
                    cur_loss_count;
                board cur_maxval_board;
                fscanf_s(in, "%d\n%lld\n%d\n%d %d %d\n", &cur_maxval,
                         &cur_maxval_board, &cur_sym_count, &cur_win_count,
                         &cur_draw_count, &cur_loss_count);
                // int total = win_count+draw_count+loss_count;
                // secs.push_back(make_pair((double)(win_count + draw_count/2) /
                // total, s));
                ...
                win_count += cur_win_count;
                loss_count += cur_loss_count;
                draw_count += cur_draw_count;
                ...
                for (int i = 0; i < dist_dist_size; i++) {
                    ...
                    fscanf_s(in, "%d\n", &x);
                    assert(x >= 0);
                    dist_dist[i - 1] += x; // dist_dist always starts at -1
                                           // (which is the draw_count)
                }
#ifdef DD
                int cur_virt_loss_val, cur_virt_win_val;
                fscanf_s(in, "%d %d\n", &cur_virt_loss_val, &cur_virt_win_val);
                REL_ASSERT(read_virt_loss_val == 0 ||
                           read_virt_loss_val == cur_virt_loss_val);
                read_virt_loss_val = cur_virt_loss_val;
                read_virt_win_val = cur_virt_win_val;
                int akey1_dist_size;
                fscanf_s(in, "%d\n", &akey1_dist_size);
                assert(akey1_dist_size ==
                       read_virt_win_val - read_virt_loss_val + 1);
...
void calc_sec_vals(map<id, wdl> &sm)
{
    ...
    for (auto it = sm.begin(); it != sm.end(); ++it) {
        ...
        if (!s.transient()) {
            assert(sm.count(-s));
            // double val2 = (double)sm[-s].win / (sm[-s].win + sm[-s].draw +
            // sm[-s].loss);

            // val = val1-val2;

            val = ((sm[s].win + sm[-s].loss) +
                   ((double)sm[s].draw / 2 + (double)sm[-s].draw / 2)) /
                  (sm[s].win + sm[s].draw + sm[s].loss + sm[-s].win +
                   sm[-s].draw + sm[-s].loss);
        } else {
            // val = 2; //ide kell majd a masik (pl. a gyerekszektorok ertekenek
            // atlagolasa? (kulon ciklusban, mert most meg nem biztos, hogy ki
            // vannak szamolva))

            val = (sm[s].win + (double)sm[s].draw / 2) /
                  (sm[s].win + sm[s].draw + sm[s].loss);
        }
    ...
    for (unsigned int i = 0; i < secs.size(); i++) {
        ...
        if (abs(secs[i].first) < eps && secs[i].first != 0) {
            assert(false); // igazabol ez nem lenne nagy baj, ha aztan itt 0-ra
                           // allitjuk
            secs[i].first = 0;
        }
    }
    ...
    auto comp = [](pair<double, id> &a, pair<double, id> &b) -> bool {
        ...
        //	-a twineket kulonbozo fajta zarojelparokkent elkepzelve helyes
        //zarojelezest kapunk 	(felteve a lentebbi elso assertet)
        if (a.first != b.first)
            return a.first < b.first;
        int al = a.second.eks() && !a.second.transient() ?
                     0 :
                     (a.second < -a.second ? -1 : 1);
        int bl = b.second.eks() && !b.second.transient() ?
                     0 :
                     (b.second < -b.second ? -1 : 1);
        // a kovetkezo ket return tartozik a masodik gondolatjelhez
        if (al == 1 && bl == 1)
            return -a.second > -b.second; //(itt a - az id-ket negalja)
                                          //(tulajdonkeppen a wu-kon akarunk
                                          //rendezest, ugyhogy kanonizalunk)
        if (al == -1 && bl == -1)
    ...
    for (unsigned int i = 0; i < secs.size() - 1; i++) {
        // assert(abs(secs[i].first - secs[i+1].first)>eps || secs[i].first==0
        // && secs[i+1].first==0); //ez nem volt igaz
        assert(abs(secs[i].first - secs[i + 1].first) > eps ||
               secs[i].first == secs[i + 1].first); // ez valojaban a rendezes
                                                    // elofeltetelehez tartozik
    ...
    for (smallest_left = first_eks; !secs[smallest_left].second.twine();
         smallest_left--)
        ;
    for (smallest_right = first_eks; !secs[smallest_right].second.twine();
         smallest_right++)
        ;

    vector<int> sec_val(secs.size());
    int left = (smallest_left + smallest_right) / 2,
        right = (smallest_left + smallest_right + 1) / 2;
    if (left == right) {
        sec_val[left] = 0;
        left--;
        right++;
    ...
    // korrigalni kell a nem-transziens eks-ek erteket 0-ra (zero-osszeguseg,
    // ld. doc)
    for (unsigned int i = 0; i < secs.size(); i++) {
    ...
    // itt azt akartuk csinalni, hogy csak a negativokra megcsinalni a
    // klaszterezest, es aztan tukrozni, de attol rossz az egesz, hogy itt nem
    // talalunk part a nem EKS tranzienseknek
    ...
    // vector<vector<double> > sse(n,vector<double>(n+1)); //sse[i][j]: az [i,j)
    // sse-je for(int i=0; i<n; i++){ 	for(int j=i+1; j<n+1; j++){ 		double sum=0;
    //		for(int k=i; k<j; k++)
    //			sum+=sec_val[k];
    //		double avg=sum/(j-i);
    //		sum=0;
    ...
    ////dp[i][j]: [0,i) intervallumot j klaszterre bontva mi az optimalis
    ///klaszterezes (ertek, az utolso klaszter eleje)
    // vector<vector<pair<double,int> > > dp(n+1, vector<pair<double,int>
    // >(num_clus+1)); for(int i=1; i<=n; i++) 	dp[i][1]=make_pair(sse[0][i], 0);
    // for(int i=1; i<=n; i++){
    ...
    for (unsigned int i = 0; i < secs.size(); i++) {
        ...
        secs[i].second.to_string().c_str(), sid.W+sid.WF-(sid.B+sid.BF));*/

        /*auto sid=sec_clus[i].second;
        fprintf_s(out, "%llf  %d  %s %d\n", sec_clus[i].first,
        sec_clus[i].first, sid.to_string().c_str(),
        sid.W+sid.WF-(sid.B+sid.BF));*/

        auto sid = secs[i].second;
        fprintf_s(out, "%lf  %s (%d)  %d\n", secs[i].first,
                  secs[i].second.to_string().c_str(),
                  sid.W + sid.WF - (sid.B + sid.BF), sec_val[i]);
    }
    fclose(out);
...
#ifdef DD
    assert(2 * virt_loss_val - 5 > sec_val_min_value); // azert kell, mert
                                                       // egyreszt korrigalas,
                                                       // masreszt levonunk
                                                       // belole egyet a
                                                       // gui_eval_elem2-ben a
                                                       // KLE-s szektorok
                                                       // ertekenel (a -5 csak
                                                       // biztonsagi, lehet,
                                                       // hogy eleg lenne -1 is)
#endif
    ...
    for (unsigned int i = 0; i < secs.size(); i++) {
        ...
        fprintf_s(out, "%d %d %d %d  %d\n", id.W, id.B, id.WF, id.BF,
                  sec_val[i]);
    }
    fclose(out);
}

void print_stats(long long &sym_count, long long &win_count,
                 long long &loss_count, long long &draw_count)
{
#ifdef DD
    init_sec_vals();

    assert(read_virt_loss_val == virt_loss_val &&
           read_virt_win_val == virt_win_val); // virt_loss_val was initialized
                                               // by init_sec_vals()
    ...
    fprintf(out, "maxval_board: %lld, toclp: %s\n", maxval_board,
            toclp3(maxval_board, maxval_board_id).c_str());
    ...
    fprintf(out, "win/draw/loss ratios: %f, %f, %f\n",
            (double)win_count / tot_count, (double)draw_count / tot_count,
            (double)loss_count / tot_count);
    ...
    for (auto it = akey1_dist.begin(); it != akey1_dist.end(); ++it)
        if (*it)
            fprintf(out, "%s %lld\n", sec_val_to_sec_name(it.ind()).c_str(),
                    *it);
    fprintf(out, "\n");
#endif
    ...
    // histogram(vector<long long>(ddv.begin()+36, ddv.end()), -1,
    // VARIANT_NAME"__disthist_above35").eps().gnuplot(false);
    histogram(vector<long long>(ddv.begin() + 81, ddv.end()), -1,
              VARIANT_NAME "__disthist_above80")
        .eps()
        .gnuplot(false);

#ifdef DD
    histogram(akey1_dist.to_vector(), virt_loss_val, VARIANT_NAME "__akey1hist")
        .gnuplot(false);
#endif
}
    ...
    for (int i = 3; i <= max_ksz; i++) {
        for (int j = 3; j <= max_ksz; j++) {
            ...
            if (r.cas() == eval_elem2::Val) {
                table[i][j] = string(r.value().key1 == 1 ? "W" : "L") +
                              tostring(r.value().key2);
            } else {
                table[i][j] = "D";
            }
            s.release_hash();
...
void ratio_table(map<id, wdl> &sm)
{
    ...
    for (int i = 3; i <= max_ksz; i++) {
        ...
        for (int j = 3; j <= max_ksz; j++) {
            ...
            sum /= 100;
            // fprintf(out, "& %.0f/%.0f/%.0f ", sm[sid].win / sum, sm[sid].draw
            // / sum, sm[sid].loss / sum);
            fprintf(out, "& %s / %s / %s ",
                    add_plus_if0(sm[sid].win / sum).c_str(),
                    add_plus_if0(sm[sid].draw / sum).c_str(),
                    add_plus_if0(sm[sid].loss / sum).c_str());
        }
        fprintf(out, "\\\\\n");
    }
