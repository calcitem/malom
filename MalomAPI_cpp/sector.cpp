...
const int sbufsize=1024*1024; char sbuf[sbufsize]; //vigyazat

Sector::Sector(::id id) : W(id.W), B(id.B), WF(id.WF), BF(id.BF), id(id), max_val(-1), max_count(-1), hash(nullptr),
#ifdef DD
...
void Sector::read_em_set(FILE* f)
{
        ...
        auto last_update = std::chrono::steady_clock::now(); // Record the last update time
        ...
        for (int i = 0; i < em_set_size; i++) {
            ...
            auto time_since_last_update = std::chrono::duration_cast<std::chrono::seconds>(now - last_update).count();
            ...
            if (time_since_last_update >= 1) {
                ...
                auto remaining_seconds = remaining_iterations * avg_seconds_per_iteration;
                int remaining_hours = remaining_seconds / 3600;
                int remaining_minutes = ((unsigned int)remaining_seconds % 3600) / 60;
                int remaining_secs = (unsigned int)remaining_seconds % 60;

                if (memoryUsageMB < 1024) {
                    printf("\rProgress: %.2f%%, Memory Usage: %.2fMB, Elapsed time: %02d:%02d:%02d, Remaining time: %02d:%02d:%02d",
                        ((float)(i + 1) / em_set_size) * 100, memoryUsageMB, hours, minutes, seconds, remaining_hours, remaining_minutes, remaining_secs);
                } else {
                    printf("\rProgress: %.2f%%, Memory Usage: %.2fGB, Elapsed time: %02d:%02d:%02d, Remaining time: %02d:%02d:%02d",
                        ((float)(i + 1) / em_set_size) * 100, memoryUsageMB / 1024, hours, minutes, seconds, remaining_hours, remaining_minutes, remaining_secs);
                }
...
void Sector::allocate(bool doinit, ::id parent){
	...
	if(doinit){
		...
		if(parent != id::null()){
			...
			//assert(mode != analyze_mode); //we don't get here in analyze mode, because we only read one sector
			LOG(", Corrections");
...
void Sector::save(){
	...
	FILE *f;
	char new_fname[255]; strcpy_s(new_fname, fname); strcat_s(new_fname, ".saving");
...
eval_elem_sym2 Sector::get_eval_inner(int i){
	...
	if(resi==SPEC){
		assert(em_set.count(i));
		int x=em_set[i];
		return x>=0 ? eval_elem_sym(eval_elem_sym::val,x) : eval_elem_sym(eval_elem_sym::count,-x);
	}else
		return resi<=MAX_VAL ? eval_elem_sym(eval_elem_sym::val,resi) : (resi<=MAX_VAL+16 ? eval_elem_sym(eval_elem_sym::sym, resi-SPEC-1) : eval_elem_sym(eval_elem_sym::count, 255-resi));
}
#endif
...
pair<sec_val, field2_t> Sector::extract(int i){
	unsigned int a = 0;
	static_assert(sizeof(a) >= eval_struct_size, "vedd nagyobbra az a-t is! (meg az intractnal is) (es nezd meg a literal 1-ek tipusait (az extendben is))) (meg minden intte castolast is)");
#ifndef WRAPPER
	...
	for(int j = 0; j < eval_struct_size; j++)
		a |= (int)read[j] << 8 * j;
#endif
	auto r = make_pair(sign_extend<field1_size, sec_val>(a & ((1 << field1_size) - 1)), sign_extend<field2_size, field2_t>(a >> field2_offset));
//#if VARIANT != STANDARD
//	r.first -= sval;
//#endif
	return r;
...
//	x.first += sval;
//#endif
	unsigned int a = (x.first&((1 << field1_size) - 1)) | ((int)x.second << field2_offset);
	for(int j = 0; j < eval_struct_size; j++)
...
void Sector::init(){
	...
	for(int h=0; h<n; h++){
		...
		for(int op=0; op<16; op++) if((1<<op)&ops){
			board a2=sym48(op,a);
			if(a2<mi){
				mi=a2;
				mh=op;
		...
		if(mi!=a){
			...
			if(B + BF <= 3 && can_close_mill(a, W, B, WF, BF)){
				assert(virt_win_val);
				set_eval(h, eval_elem2( virt_win_val - sval, 1 )); //intellisense bug, if we use {}
			} else {
				//int c=CHILD_COUNT(a,W,B,WF,BF);

				/*c*=orbitsize(a);
				set_eval(h,eval_elem(c ? eval_elem::count : eval_elem::val ,c));*/

				set_eval(h, eval_elem2{ 0 }); //(count) ezt a 0-t fogja novelgetni a forditott gyerekszamolas
			}
		}
	}
#endif
...
short_id::operator Sector*(){return sector_objs[sid];}
short_id::short_id(Sector *s):sid(s->sid){}
short_id::short_id(){}
short_id::short_id(char sid):sid(sid){}
short_id::operator id(){return ((Sector*)(*this))->id;}

#endif


void Sector::allocate_hash(){ // and read em_set (should be renamed)
