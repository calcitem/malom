...
map<id, sec_val> sec_vals; //vigyazat: STONE_DIFF esetben vannak benne nagyon nem letezo szektorok is
#ifndef STONE_DIFF
...
void init_sec_vals(){
	...
	int n;  fscanf_s(f, "%d\n", &n);
	for(int i = 0; i < n; i++){
		int w, b, wf, bf, v;  fscanf_s(f, "%d %d %d %d  %d\n", &w, &b, &wf, &bf, &v);
	...
	for(auto sv : sec_vals){
		if(sv.second){ //not NTREKS if DD  (if not DD, then only the virt sectors (which btw don't get here) are non-0)
			assert(!inv_sec_vals.count(sv.second)); //non-NTREKS sec_vals should be unique
	...
	for(auto s : sector_list){
		...
		auto xx = sec_vals[s];
		assert(s.transient() || sec_vals[s] == -sec_vals[-s]); //wus are zero-sum
	}
#endif
}
