...
vector<id> std_mora_graph_func(id u){
	...
	for(auto it=v.begin(); it!=v.end(); it++)
		if(it->B + it->BF >=3 &&
		   it->B >= 0) //ez tulajdonkeppen csak a kezdoallast kezeli, ld. doc
			r.push_back(*it);
...
vector<id> graph_func(id u, bool elim_loops){
	...
	if(elim_loops) sr.erase(u); //hurokel kiszurese
...
void add_adj(wu &wu, id id){
	...
	for(auto it=e.begin(); it!=e.end(); ++it)
		if(wus[*it]!=&wu) //hurokelek kiszurese (vigyazzunk, hogy csak pointer szerint masolgassuk a wu-kat!)
			if(wu.parents.insert(wus[*it]).second) //kiszurodnek a parhuzamos elek
				wus[*it]->child_count++;
}
...
void init_wu_graph(){
	//a sector_list-beli sorrend hatarozza meg, hogy a wu-k ket szektora kozul melyik az elsodleges, tehat mindig a kisebb id-ju
	...
	for(auto it=wus.begin(); it!=wus.end(); ++it){ //(nem baj, hogy ketszer megyunk a twineken)
		auto &wu=*(it->second);
