...
using sec_val = short int;
	...
	//a 256 ertekbol lefoglalunk 1-et a specnek, 1-et a val0-nak, 1-et a count0-nak (ezert 253-...)
	#define MAX_VAL 178
...
const board mask24 = (1 << 24) - 1; //azert nem lehet int, mert van olyan hasznalata, hogy pl. mask24<<cps (amugy ez sehol sem lassit valszeg)
...
struct val {
	...
	val() {}
	val(sec_val key1, int key2) : key1(key1), key2(key2) { assert(key1); /*nem count*/ }

	bool operator==(const val &o) const { return key1 == o.key1 && key2 == o.key2; }
	
	//tuple<sec_val, signed char, int> tr() const { return make_tuple(-abs(key1), key1 >= 0 ? -1 : 1, key2); }
	//tuple<sec_val, int, signed char> tr() const { return make_tuple(-abs(key1), key2, key1 <= 0 ? -1 : 1); } //ezzel az a gond, hogy a bucketekbe rakasnal kulonbozo iranyokba korrigalunk a key2 elojeletol fuggoen
	tuple<sec_val, int> tr() const { return make_tuple(-abs(key1), key2); }
	bool operator<(const val &o) const { return tr() < o.tr(); } //a feldolgozas sorrendje
...
template<class T>
string tostring(T x){ stringstream ss; ss << x; return ss.str(); }


#ifdef NDEBUG
#define REL_ASSERT(_Expression) \
	(void)( (!!(_Expression)) || ( \
	LOG("REL_ASSERT failure: %s\n", ((string)#_Expression + "  " + __FILE__ + "  " + tostring(__LINE__)).c_str()), \
	abort(), 0) )
